    ...
    connection.query('SELECT attempts from login where username=?',[n1] ,function (error, results, fields) { 
      ...
      if(lent>=3){
        ...
        //   for(i=0;i<result.length;i++){
          res.write("</center></head><body><center><br><br><h2><a href='silly-stonebraker-e437ea.netlify.com/forgotpassword'> Follow this link to reset password</a><h2> ");
  ...
  app.post("/ret", function(req, res) {
    ...
    //console.log(n);
     ...
     connection.getConnection(function(err) {
        ...
        //   var i;
        //   for(i=0;i<result.length;i++){
          res.write("</center></head><body><center><br><br><h2><a href='silly-stonebraker-e437ea.netlify.com/done'> Reset Link </a><h2> ");
          res.write("</center</body></html>");
          res.end();
  ...
  connection.query('SELECT * from studdetails where dept=? and section=?',[n1,n2] ,function (error, results, fields) {
    var length=results.length;
     if(length<=0)
     {
      ...
      html+='</tbody></table></p><br><br><a href="silly-stonebraker-e437ea.netlify.com/studpage" class="btn btn-info btn-lg"><span class="glyphicon glyphicon-circle-arrow-left"></span> Back</a></body></html>';
        res.send(html);
     }
    });
    res.status(200);
